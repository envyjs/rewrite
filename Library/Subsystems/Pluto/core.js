  ...
  try {
    ...
    const GlobalLib = {
      ...
      loadLibrary: async function (lib) {
        ...
        if (lib === "WindowSystem") {
          return await Core.startPkg("../../windowSystem.js", false, true);
        }
    ...
    GlobalLib.icons = (await import("./icons.js")).default;
    const strings = (await import("./strings.js")).default;
      ...
      startPkg: async function (url, isUrl = true, force = false) {
        try {
          ...
          if (isUrl === false) {
            ...
            pkg = await import("../../../System/Pluto/" + url.replace(":", "/") + ".js");
          }
              ...
              if (
                ...
                if (force === false)
                  modalResult = await new Promise((resolve, reject) => {
                    setTimeout(() => {
                      let x = new Audio("../../Library/System Audio/alert.wav");
                      x.volume = 0.75;
                      x.play();
                    }, 100);
                    Modal.modal(
                      ...
                      `${getString("core_appAccessControl_description", {
                        ...
                        .map(
                          (m) =>
                            ...
                            )}<br><span class="label">${privileges[m].authorNote !== undefined
                              ? `${getString(
