import Win from "../../Library/windowSystem.js";
...
export default {
  ...
  start: async function (Root) {
    ...
    myWindow.window.classList.add("terminal")
    ...
    let Terminal = new Html("div")
      .class("no-ui")
      .html(
        /*html*/ `<div><div class="output">Envy [Version ` + Root.systemInfo.version +`] (` + Root.systemInfo.pr + `)<br>Copyright (C) Envy Group, Zeon.dev 2022-2025<br><br></div></div><div class="userInput"><div style="display:flex;" id="inputDiv"><span class="prompt">${Term.prompt}</span><div class="none input" contenteditable="true"></div></div></div>`
      );
    ...
    async function runCommand(elm, input = "") {
      ...
      if (Term.commandLine === false) {
        ...
        switch (cmd) {
          case "fetch":
            appendOutput("@@@@@@@@   OS: "+ Root.systemInfo.string+ "\n @      @  Host: " + window.location.hostname +  "\n  @      @ Kernel: " + Root.systemInfo.version+ "\n @      @  WM: WSWindow\n@@@@@@@@   Terminal: Envy Terminal")
          case "echo":
