    ...
    Library: {
      Subsystems: {
        Pluto: {
          panics: {
            "README.MD":
              "This folder is designed to help store logs when the Pluto subsystem crashes.",
          },
          config: {
...
function fixString(str) {
  // Envy-specific Pluto subsystem fallback
  if (str.startsWith("Root/Pluto")) {
    str = str.replace("Root/Pluto", "Root/Library/Subsystems/Pluto");
  }
  if (str.startsWith("Registry/")) {
    str = str.replace("Registry/", "Root/Library/Subsystems/Pluto/Registry/");
  }
  return str;
}
